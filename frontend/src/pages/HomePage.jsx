      ...
      <div style={{ position: 'relative', flexShrink: 0 }}>
        <div style={{
          ...
          background: isSelected
            ? 'linear-gradient(135deg,#7c65f6,#5a45d4)'
            : 'linear-gradient(135deg,#2a2a3a,#1c1c28)',
          display:    'flex',
          alignItems: 'center',
...
export default function HomePage() {
  ...
  const handleMessage = useCallback((message) => {
    // If it's for the open conversation, append it
    setMessages((prev) => {
      ...
      if (
        ...
        (message.receiverId === selectedUser?.id && message.senderId   === currentUser?.id)
      ) {
        return [...prev, message]
      }
      return prev
    ...
    // Update unread badge if message is from someone else and not the open chat
    if (
  ...
  const handleSend = () => {
    ...
    const sent = sendMessage(selectedUser.id, content)
    if (!sent) {
      // WS not ready — fallback to REST
      messagesApi.send({ receiverId: selectedUser.id, content })
        .then(({ data }) => {
  ...
  const groupedMessages = () => {
    ...
    messages.forEach((msg) => {
      ...
      if (label !== lastDate) {
        groups.push({ type: 'separator', label, id: `sep-${msg.createdAt}` })
        lastDate = label
      }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            gap:          '10px',
            padding:      '10px 12px',
            background:   'rgba(124,101,246,0.08)',
            borderRadius: '10px',
          ...
          {selectedUser ? (
            <>
              ...
              <div>
                ...
                <div style={{
                  fontSize: '12px',
                  color:    onlineIds.includes(selectedUser.id) ? '#4fd1c7' : 'var(--text-muted)',
                }}>
